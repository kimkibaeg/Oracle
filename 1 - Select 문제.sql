...
*/
 ...
 select eno 사원번호, ename 사원이름, salary 월급, commission 보너스, salary * 12 + nvl(commission,0) 연봉
 from employee
...
where commission is not null;
 ...
 select eno 사원번호, ename 사원이름, job 직책, salary 월급
 from employee
 where job != 'SALESMAN';
 ...
 select eno 사원번호, ename 사원이름, job 직책, salary 월급, hiredate 입사날짜
 from employee
 ...
 select eno 사원번호, ename 사원이름, job 직책, salary 월급, dno 부서번호
 from employee
