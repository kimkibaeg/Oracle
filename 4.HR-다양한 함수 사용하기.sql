...
/*
1. 문자를 처리하는 함수 : UPPER : 소문자를 대문자로 변환해주는 함수
...
select 'Oracle Mnia', 'Mania' from dual;

select concat ('Orcle', 'Mania')as 연결됨 from dual;
...
select substr ('Oracle Mania', 4,3) from dual;  -- index : 1 번부터 시작
...
select (instr('Oracle Mania', 'a')) from dual ; -- 3 <== a가 위치한 index
...
select '030802-123456' 주민번호, LPAD ('030802-1', 20, '*')as "LPAD" from dual;

select '030802-123456' 주민번호, RPAD ('030802-1', 20, '*')as "RPAD" from dual;
...
-- 2. 숫자를 처리하는 함수 : ROUND : 반올림하는 함수, TRUNC : 특정 자리수에서 잘라내어버림. MOD : 나머지 값만 출력

-- ROUND : 특정 자릿수에서 반올림, 5이상 반올림, 5미만은 잘라내어버림.
/*
    ROUND (실수) : 소숫점 뒷자리에서 반올림
    ROUND (실수, 소수점자리수) : 
        - 소숫점 자리 (양수) : 소숫점 기준으로 오른쪽으로 이동해서, 그 뒷자리에서 반올림 됨  <== 주의
        - 소숫점 자리 (음수) : 소숫점 기준으로 왼쪽으로 이동해서, 그 자리에서 반올림 됨.  <== 주의
*/
...
-- TRUNC : 특정 자리에서 잘라내 버림.  <== 특정 날짜를 연산.
...
-- 입사일에서 오늘날까지 총 근무 일수를 구함. date 컬럼    (sysdate - hiredate)
desc employee;

-- 총근무일수 = trunc(오늘날짜 - 입사날짜)
...
-- 현재 까지의 근무개월수를 출력 : months_between (날짜, ㄱ날짜)  : 두 날짜 사이의 개월 수 출력
...
select to_date ('1981/01/01', 'YYYY/MM/DD') + 100 "100일 후 날짜",
    add_months(to_date('1981/01/01', 'YYYY/MM/DD'), 100) + 100 "100개월 후 날짜"
from dual;

select to_date(770814, 'YYMMDD') from dual

-- 자신의 생일에서 현재까지 몇개월 살았는지? 몇일 살았는지 출력 <== months_between (현재날짜, 생일) : 개월 수
    -- sysdate - 생일 (date)
