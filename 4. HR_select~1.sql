...
-- 특정 컬럼을 여러번 출력 하기, 컬럼에 연산을 할 수 있다. 컬럼들이 바뀔 수 있다.
...
-- 테이블 구조를 확인 하기, desc 테이블 명, 실제 컬럼명, null 허용여부, 자료형(number : 정수, 실수, char, varchar2 : 문자열)
...
-- nvl (컬럼명, 0)  : null 을 처리하는 함수, 컬럼의 값 중 null 을 0으로 반환하는 함수

select eno 사원번호, ename 사원명, salary 월급, commission 보너스,     -- null 처리가 안됨
...
-- SELECT 문의 전체 내용, SQL : 구조화 된 질의 언언
/*
    SELECT 컬럼명
    FROM 테이블명[뷰명]
    WHERE 조건
    ...
    ODER BY 컬럼명 ASC[DESC]       <== 컬럼을 정렬해서 출력, ASC : 내림차순정렬, DESC : 오름차순정렬
*/

-- where 조건 사용하기 : 비고 연산자 (NUMBER 타입의 컬럼에서 사용) :
        -- = : 같다, >, <, >=, <=     // 같지 않다 : <>, !=, ^=
        -- or
        -- and
        -- between
...
-- 직책이 manager 이면서 연봉이 2000 이상인 사원의 사원번호, 사원명, 월급을 출력

select eno 사원번호, ename 사원명, salary 월급, job 직책
from employee
where job = 'MANAGER' and salary > 2000 ;

-- 비교 연산자를 date, varchar, char : 81년도에 입사한 사원을 모두 출력
...
-- like : 문자열에서 특정 값만 검색 : % : 모든것이 와도 상관없다,  _ : 한글자가 어떤 것이 와도 상관없다.
...
select commission 수능점수      -- null : 비어있는 상태,
from employee;

desc employee;

-- 테이블의 전체 레코드를 정확하게 출력 : not null 컬럼을 count(컬럼형), count(*)

-- 특정 컬럼을 정렬해서 출력 : order by 컬럼명 ASC{DESC}
  -- ASC : 오름 차순 정렬, A ====> Z, 가 ====> 하, 1 ====> 9
  -- DESC : 내림 차순 정렬 : Z ====> A, 하 ====> 가, 9 ====> 1
  
-- 월급 (salary) 컬럼을 오름차순 정렬
select *
from employee
order by salary asc;        -- asc 는 생략 할 수 있다.
...
-- 정렬할 컬럼을 where 와 같이 사용할 때 order by 는 where 절 다음에 처리
select*
from employee
where salary > 1500
order by salary desc;

-- order by 에서 여러 컬럼을 정렬 할 때, 앞의 컬럼을 모두 정렬하고, 같은 값이 존재할 때 같은 값에 대해서 뒤의 컬럼을 정렬
  -- 답변형 게시판 만들 때 사용됨.....
  ...
  -- job : 오름 차순 정렬, dno : job 의 동일한 값에 대해서 dno  를 오름차순,
