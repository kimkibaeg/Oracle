/*
    DDL : Create, Alter, drop   <-- 객체 (테이블, 뷰, 함수, 저장프로시져, 트리거, 스퀸스...)
    
    - 테이블 생성 후, 기존에 생성된 테이블에 대해서 빈번하게 유지 보수 (Alter Table)
      - 1. 컬럼에 자료형을 변경 : varchar(50)  ===> varchar(100)
      - 2. 컬럼의 이름을 변경, 컬럼을 삭제, 컬럼을 추가
      - 3. 컬럼의 제약조건을 추가, 삭제 (Primary Key, Unique, Not Null, Check, Default)
*/

-- 실습을 위한 테이블 복사 : 컬럼명, 값만 복사됨, 컬럼에 부여된 제약 조건은 복사되지 않는다.
create table emp60
as
...
-- 데이터 사전 : user_ ,  user_constarints : 데이터 베이스에 존재하는 모든 테이블에 대한 제약조건을 출력
...
-- 1. Alter Table 을 사용해서 기존 테이블에 컬럼을 추가 : birth, email(varchar2(100))
        -- 테이블에 컬럼을 추가시 null 로 처리 후 컬럼을 추가해야함.
...
select * from user_constriants where table_name in ('DEPARTMENT', 'EMPLOYEE') ;
...
-- 제약 조건 이름을 생략한 경우 : Oracle 에서 sys_* 로 시작되는 임의의 이름을 넣음.
alter table dept60 
add primary key (dno) ;
...
alter table dept60
drop constraint SYS_C008456;

-- Foreign Key 제약 조건을 넣기 위해서는 
    -- 1. 부모 테이블의 컬럼의 : Primary Key, Unique, 2. 참조 컬럼의 자료형이 비슷하거나 같아야함.
...
-- 6-5. NOT NULL 제약 조건 : 컬럼에 NULL 없도록 설정, 반드시 값이 입력이 되어야 한다.
        -- 컬럼의 값이 NULL 이 없어야 한다. 있으면 오류 발생.
...
-- 6-6. default : 제약 조건이 아니다. 제약조건 이름을 부여 할 수가 없다.
        -- 컬럼의 값을 넣지 않을 경우 default 로 설정된 값이 자동으로 들어감.
        -- default 에 설정을 제거하기 위해서는 default null
